...
function renderVerificatorCard(item) {
    return `
        <article class="rounded-lg border border-slate-700/80 bg-slate-800/50 p-3">
            ...
            ${item.evidenceQuote ? `
                <blockquote class="mb-2 border-l-2 border-indigo-500/50 pl-3 text-sm italic text-slate-400">
...
function renderAgentResponse(data) {
    ...
    const verifications = (data.verificatorResponses ?? [])
        .map(renderVerificatorCard)
        .join('');
...
function createErrorBubble(message) {
    return createMessageBubble('agent', `<p class="text-red-400">${escapeHtml(message)}</p>`);
}
