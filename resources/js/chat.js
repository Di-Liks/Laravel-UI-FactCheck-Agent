...
function renderVerificatorBrief(item) {
    ...
    const details = `
        <div class="mt-3 space-y-2 border-t border-slate-700/60 pt-3">
            ...
            ${item.evidenceQuote ? `
                <blockquote class="border-l-2 border-indigo-500/50 pl-3 text-sm italic text-slate-400">
                    ${escapeHtml(item.evidenceQuote)}
                </blockquote>
            ` : ''}
...
function renderDecomposerSection(responses) {
    ...
    const blocks = responses.map((decomp) => {
        const claims = (decomp.atomicClaims ?? [])
            .map(
                (c) =>
                    `<li class="text-sm text-slate-400"><span class="text-slate-500">${escapeHtml(c.type)}</span> — ${escapeHtml(c.text)}</li>`
            )
            .join('');
...
function renderAgentResponse(data) {
    ...
    const expandedSummary = `
        <div class="mt-3 space-y-3 border-t border-white/10 pt-3">
            ${parsed.analysis ? `
                <div class="whitespace-pre-wrap text-sm leading-relaxed text-slate-300">
                    ${escapeHtml(parsed.analysis)}
                </div>
            ` : ''}
            ${data.summatorResponse && !parsed.analysis ? `
                <div class="whitespace-pre-wrap text-sm leading-relaxed text-slate-300">
                    ${escapeHtml(data.summatorResponse)}
                </div>
            ` : ''}
...
function createTypingIndicator() {
    ...
    wrapper.innerHTML = `
        <div class="flex items-center gap-1.5 rounded-2xl rounded-bl-md border border-slate-700/80 bg-slate-800 px-4 py-3">
            <span class="h-2 w-2 animate-bounce rounded-full bg-slate-500 [animation-delay:-0.3s]"></span>
            <span class="h-2 w-2 animate-bounce rounded-full bg-slate-500 [animation-delay:-0.15s]"></span>
...
function createErrorBubble(message) {
    return createMessageBubble(
        'agent',
        `<div class="rounded-xl border border-red-500/50 bg-red-500/10 p-4"><p class="text-red-400">${escapeHtml(message)}</p></div>`
    );
}

export function initFactCheckChat() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            messagesEl.appendChild(
                createMessageBubble('agent', renderAgentResponse(data))
            );
        } catch (err) {
            typing.remove();
            ...
            messagesEl.appendChild(createErrorBubble(message));
        } finally {
            setLoading(false);
            scrollToBottom();
            input.focus();
